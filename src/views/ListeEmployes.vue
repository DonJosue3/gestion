<template>
    <section class="wrapper">
        ...
        <div class="table-container">
            <div class="table-wrap-body">
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="employe, id in listeEmployes" :key="id"
                            ...
                            <td class="act">
                                <button v-if="checkConge(employe)" @click="openAddConge(i)" class="btn-sm btn-primary">
                                    <i class="mdi mdi-pencil"></i>
                                    Ajouter Congé
                                </button>
                                <button @click="modifierEmploye(employe)" class="btn-sm btn-primary">
                                    <i class="mdi mdi-pencil"></i>
                                    Modifier
                                </button>
        ...
        <ModalAddEmploye v-if="add_employe_shown" @close="close" @addEmploye="addEmploye"></ModalAddEmploye>
        <AddConge v-if="add_conge_shown" @close="close" @addCongeEmitted="addConge" />
        <ModifierEmploye v-if="edit_employe_shown" @close="close" @editEmploye="replaceEmploye"
            :employeProp="employeObj" />
...
export default {
    ...
    methods: {
        ...
        }, replaceEmploye(newEmploye) {
            const index = this.listeEmployes.findIndex(employe => employe.id === newEmploye.id);
            if (index !== -1) {
                // Remplacer l'objet existant par le nouvel employé complet
                this.listeEmployes[index] = { ...newEmploye };
            }

            // Fermer le composant de modification
            this.edit_employe_shown = false;
        },
